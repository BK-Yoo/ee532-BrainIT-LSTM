...
def random_initialization(size):
    return (np_rng.standard_normal(size) * 0.001 / size[0]).astype(theano.config.floatX)
...
class Embedding(Layer):
    """
    An Embedding layer than contains an embedding matrix and
    takes integers as input(input is composed of integer, because of conversion)
    and returns slices(e.g. word vectors) from its embedding matrix
    """
    ...
    def activate(self, x):
        """
        return row of embedding_matrix,
        it means generating 'word vector'(1 * hidden_size)
        """
...
class LSTM(RNN):
    ...
    def create_variables(self):
        ...
        # ndim of this variable is 1(vector)
    ...
    def postprocess_activation(self, x, *args):
        """
        :param x: cancatenation of real input and hidden states.
        :param args:
        :return: x_input
        """
        if x.ndim > 1:
    ...
    def activate(self, x, h):
        # print "lstm input x dimension: ", x.ndim
        # print "lstm input h dimension: ", h.ndim
...
class StackedCells(object):
    ...
    def create_layers(self, layer_sizes, activation_type, celltype):
        ...
        for k, layer_size in enumerate(layer_sizes):
            ...
            # the number of nodes in current layers are input size to next layer.
    ...
    def forward(self, x, prev_hiddens=None, dropout=None):
        ...
        # Initialize prev_hiddens variable.
...
def create_optimization_updates(cost, params, updates=None, max_norm=5.0,
    ...
    return updates, gsums, xsums, rho, max_norm
